...
app.post("/api/jamaah", async (req, res) => {
  ...
  try {
    ...
    // Sanitize tanggal_daftar: if it's not a valid date string, set to null or current date
    let sanitizedTanggalDaftar = null;
    if (tanggal_daftar && tanggal_daftar !== "TIDAK TERDETEKSI") {
      const date = new Date(tanggal_daftar);
      if (!isNaN(date.getTime())) {
        sanitizedTanggalDaftar = date.toISOString().split('T')[0];
      }
    }
      ...
      if (existingJamaah) {
        ...
        const { data: updateData, error: updateError } = await supabase
          ...
          .update({ 
            ...
            alamat, 
            phone, 
            tanggal_daftar: sanitizedTanggalDaftar, 
            foto,
    ...
    if (!jamaahId) {
      const { data: jamaahData, error: jamaahError } = await supabase
        ...
        .insert([
          { 
            nomor_porsi: (nomor_porsi && nomor_porsi !== "TIDAK TERDETEKSI") ? nomor_porsi : `${Date.now()}`, 
            nama, 
            nama_ayah, 
            alamat, 
            phone, 
    ...
    // Sanitize nomor_dokumen
    const sanitizedNomor = (nomor && nomor !== "TIDAK TERDETEKSI") ? nomor : `${Date.now()}`;
